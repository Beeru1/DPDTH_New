...
public class DPCreateAcountRdbmsDB2 extends DPCreateAccountDaoImpl {
	...
	public DPCreateAcountRdbmsDB2(Connection conn) {
		super(conn);
...
//				long accountId = reportInputDto.getSessionContext().getId();
				...
				try {
					...
					if (0 != parentId) {
						...
						sql.append(RDBMSQueries.SQL_SELECT_ALL_ACC_CHILD_DISTRIBUTOR);
						sql.append("and details.account_level in ( select level_id from VR_ACCOUNT_LEVEL_MASTER where acc_level > ("+
								...
								sql.append( " AND LOGIN.STATUS in ("+status+")  AND DETAILS.CIRCLE_ID = "+circleId);
						}
						else 
						{
							sql.append(RDBMSQueries.SQL_SELECT_ALL_ACC_CHILD_DISTRIBUTOR
									+ " AND LOGIN.STATUS in ("+ status+")  AND DETAILS.CIRCLE_ID = "+circleId);
						}
					...
					if (searchType != null) {
						logger.info("The LEVEL ID    ---------------  "+reportInputDto.getSessionContext().getAccountLevel());
						// search according to mobile no
						if (searchType.trim().equalsIgnoreCase(
								Constants.SEARCH_TYPE_MOBILE)) {
							if((reportInputDto.getSessionContext().getAccountLevel().equalsIgnoreCase((Constants.DISTRIBUTOR_ID-1)+""))){
								sql.append(" AND DETAILS.RETAILER_LAPU LIKE ? ");
							}
							else{
								sql.append(" AND DETAILS.MOBILE_NUMBER LIKE ? ");
							}
						...
						query.append(sql);
						query.append("  order by account_level) a)b with ur");
						...
						//ps.setString(paramCount++, status);//Neetika
						if(roleName){
							ps.setLong(paramCount++, parentId);
							ps.setLong(paramCount++, parentId);
						}
					...
					int i=1;
					while (rs.next()) {
						//System.out.println("finds the records.........");
						accountDto = new DPCreateAccountDto();
						// new change group name added
						accountDto.setAccountId(rs.getInt("ACCOUNT_ID"));
						accountDto.setAccountCode(rs.getString("LOGIN_NAME"));
		...
		try {
			...
			if (0 != parentId) {
				...
				sql.append("and details.account_level in ( select level_id from VR_ACCOUNT_LEVEL_MASTER where acc_level > ("+
					...
					sql.append(RDBMSQueries.SQL_SELECT_ALL_ACC_CHILD_DISTRIBUTOR 
							+ " AND LOGIN.STATUS =  ?  AND DETAILS.CIRCLE_ID = "+circleId);
				}
				roleName = reportInputDto.getSessionContext().getRoleList().contains("ROLE_AD");
				if(roleName){
					sql.append(" and (DETAILS.PARENT_ACCOUNT= ? OR DETAILS.ROOT_LEVEL_ID= ?) ");
			...
			if (searchType != null) {
				...
				System.out.println("levelId:::::"+parentId);
				System.out.println("levelId:::::"+levelId);
				System.out.println("levelId:::::"+status);
				//System.out.println("levelId:::::"+levelId);
		...
		try {
			...
			if (circleId != 0) {
				...
				if (circleName.equalsIgnoreCase("NATIONAL")) {
					...
					sql.append(" AND LOGIN.STATUS in ("+status+")");
				} else {
					/** if circle user, show records of respective circle */
					sql.append(queryMap.get(SQL_SELECT_ALL_ACC_COUNT_KEY));
					sql.append(" AND DETAILS.CIRCLE_ID = ");
