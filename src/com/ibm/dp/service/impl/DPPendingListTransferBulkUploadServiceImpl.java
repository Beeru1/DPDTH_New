...
public class DPPendingListTransferBulkUploadServiceImpl implements DPPendingListTransferBulkUploadService
{
	Logger logger = Logger.getLogger(DPPendingListTransferBulkUploadServiceImpl.class.getName());
	public List uploadExcel(InputStream inputstream) throws DPServiceException
	{
		List list = new ArrayList();
		Connection connection = null;
		DPPendingListTransferBulkUploadDao bulkupload = null; 
		try
		{
			...
			bulkupload = DAOFactory.getDAOFactory(Integer.parseInt(ResourceReader.getCoreResourceBundleValue(Constants.DATABASE_TYPE)))
														.getPendingListTransferExcelDao(connection);
	...
	public String updatePendingListTransfer(List list, String userId) throws DPServiceException
	{
		...
		try
		{
			...
			bulkupload = DAOFactory.getDAOFactory(Integer.parseInt(ResourceReader.getCoreResourceBundleValue(Constants.DATABASE_TYPE)))
														.getPendingListTransferExcelDao(connection);
			strMessage = bulkupload.updatePendingListTransfer(list,userId);
			connection.commit();
		}
		catch (Exception e) 
		{
