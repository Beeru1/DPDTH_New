...
public interface BusinessUserInterface {
	...
	.append("STATUS = ?,FOS_CHECK_REQ = ? ,FOS_ACTV_CHK = ? ,ALL_SERVICE_CLASS = ? ,INCLUDE_SERVICE = ? ,EXCLUDE_SERVICE = ? ,LOC_ID = ? ,HUB_CODE = ?,IPHONE_ACT_RIGHTS=?,PARENT_ID=? WHERE DATA_ID = ?")
	...
	.append("select * from ")
	//.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append("VR_ACCOUNT_LEVEL_MASTER where LEVEL_ID>(select LEVEL_ID from VR_ACCOUNT_LEVEL_MASTER where LEVEL_NAME='TSM') and " +
			" HIERARCHY_ID=(select HIERARCHY_ID from VR_ACCOUNT_LEVEL_MASTER where LEVEL_NAME='TSM') ").toString();
	
	public static String SEARCH_USER_TYPE_ALL_ORDER_BY = " ORDER BY LEVEL_ID";
	
	
	public static String GET_CHILDERN_LIST = new StringBuffer(500)
	.append("select DATA_ID,USER_NAME,USER_CODE,MASTER_ID , STATUS ,PARENT_ID , LOC_ID FROM ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_BUSINESS_USER_DATA WHERE LOC_ID = ? AND PARENT_ID = ? AND STATUS=?").toString();
	...
	.append("select DATA_ID,USER_NAME,USER_CODE,MASTER_ID , USER_DATA.STATUS ,PARENT_ID , LOC_ID, MOBILE_NO FROM ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_BUSINESS_USER_DATA USER_DATA LEFT OUTER JOIN ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_REGISTERED_NUMBER ON BUS_USER_ID = DATA_ID ")
	.append(" WHERE LOC_ID = ? AND MASTER_ID = ? AND USER_DATA.STATUS=?").toString();
	
	public static String GET_MSISDN = new StringBuffer(500)
	.append("SELECT MOBILE_NO FROM ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_REGISTERED_NUMBER WHERE BUS_USER_ID = ? ").toString();
	
	
	public static String GET_SUB_BIZ_LIST_FOR_MOVE = new StringBuffer(500)
	.append("select ad.ACCOUNT_ID DATA_ID,ad.ACCOUNT_NAME USER_NAME,ad.ACCOUNT_ID USER_CODE,ad.ACCOUNT_LEVEL MASTER_ID ,lm.status,ad.CITY_ID PARENT_ID,ad.LAPU_MOBILE_NO MOBILE_NO from ")
	//.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(" vr_account_details ad,vr_login_master lm ")
	//.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(" where lm.login_id = ad.account_id and circle_id=? and lm.status=?").toString();
	
	public static String GET_SUB_BIZ_LIST_FOR_AVAILABLELIST = new StringBuffer(500)
	.append("select DATA_ID,USER_NAME,USER_CODE,MASTER_ID , STATUS ,PARENT_ID , LOC_ID , CONTACT_NO FROM ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_BUSINESS_USER_DATA WHERE PARENT_ID = ? AND CIRCLE_CODE = ? ").toString();
	
	public static String GET_PARENT_LIST = new StringBuffer(500)
	.append("select DATA_ID,USER_NAME FROM ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	...
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_REGISTERED_NUMBER ON BUS_USER_ID = DATA_ID ")
	.append(" WHERE HUB_CODE = ? AND MASTER_ID = ? AND USER_DATA.STATUS=?").toString();
	
	
	public static String GET_PARENT_LIST_FOR_ED = new StringBuffer(500)
	.append("select DATA_ID,USER_NAME FROM ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	...
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_REGISTERED_NUMBER ON BUS_USER_ID = DATA_ID ")
    .append(" WHERE CIRCLE_CODE = ? AND MASTER_ID = ? AND USER_DATA.STATUS=?").toString();
	
	public static String GET_PARENT_LIST_FOR_CEO_SALESHEAD = new StringBuffer(500)
	.append("select DATA_ID,USER_NAME FROM ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	...
	.append("Update ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_BUSINESS_USER_DATA SET LOC_ID = ?, CIRCLE_CODE =? , UPDATED_BY=?, UPDATED_DT = current timestamp WHERE  DATA_ID IN (<DATA_LIST>)").toString();
	
	public static String UPDATE_BIZ_LIST_PARENT_IDS = new StringBuffer(500)
	.append("Update ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_BUSINESS_USER_DATA SET PARENT_ID =? WHERE  DATA_ID IN (<DATA_LIST>)").toString();
	
	public static String GET_DOWN_HIERARCHY_BIZ_USERS = new StringBuffer(500)
	.append("WITH temptab(DATA_ID , USER_NAME , MASTER_ID , PARENT_ID , STATUS , LOC_ID, CONTACT_NO ) AS ") 
	.append("( Select root.DATA_ID AS DATA_ID, root.USER_NAME AS USER_NAME, ")		
	.append("root.MASTER_ID AS MASTER_ID, root.PARENT_ID AS PARENT_ID, ")
	.append("root.STATUS AS STATUS, root.LOC_ID AS LOC_ID, root.CONTACT_NO AS CONTACT_NO from ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_BUSINESS_USER_DATA root ")
	.append("where root.DATA_ID IN (<DATA_LIST>) ")
	.append("UNION ALL ")
	.append("Select sub.DATA_ID AS DATA_ID, sub.USER_NAME AS USER_NAME, ")
	.append("sub.MASTER_ID AS MASTER_ID, sub.PARENT_ID AS PARENT_ID, sub.STATUS AS STATUS, ")
	.append("sub.LOC_ID AS LOC_ID, sub.CONTACT_NO AS CONTACT_NO from ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_BUSINESS_USER_DATA  sub, temptab super ")
	.append("where  sub.parent_ID= super.Data_ID ")
	.append(") ")
	.append("SELECT * from temptab").toString();


	public static String SELECT_BIZ_LIST = new StringBuffer(500)
	.append("SELECT bu.DATA_ID,bu.USER_NAME,bu.USER_CODE,bu.MASTER_ID , bu.STATUS ,bu.PARENT_ID , bu.LOC_ID ,rn.MOBILE_NO ")
	.append("FROM ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_BUSINESS_USER_DATA bu, ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append(".V_REGISTERED_NUMBER rn WHERE bu.CIRCLE_CODE = ? AND rn.BUS_USER_ID=bu.DATA_ID AND bu.DATA_ID IN (<DATA_LIST>)").toString();
	...
	.append("select LEVEL_ID MASTER_ID,LEVEL_NAME BASE_LOC,ACC_LEVEL,HIERARCHY_ID,MOVE_FLAG from ")
	//.append(SQLConstants.DISTPORTAL_SCHEMA)
	.append("VR_ACCOUNT_LEVEL_MASTER WHERE LEVEL_ID = ? ")
	.append(" WITH UR").toString();
	...
	.append(" mt.USER_TYPE ,mt.BASE_LOC, INCLUDE_SERVICE ,EXCLUDE_SERVICE ,CREATED_BY ,UPDATED_DT ,UPDATED_BY ,CREATED_DT,IPHONE_ACT_RIGHTS from  ")
	.append(SQLConstants.DISTPORTAL_SCHEMA)
