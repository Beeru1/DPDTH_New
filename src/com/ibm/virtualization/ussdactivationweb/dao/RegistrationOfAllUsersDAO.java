...
public class RegistrationOfAllUsersDAO {
	...
	public String insert(RegistrationOfAllDTO form, int intUserId)
		...
		try {
			...
			if (status.equalsIgnoreCase(Constants.VALID)) {
			...
			if (strInput.length() != 0) {
				...
				for (int i = 0; i < arrayList.size(); i++) {
					...
					if (arrayList.get(i).toString().length() == 0) {
						return RETURN_CONSTANT = Constants.SERVICE_CLASS_EXSISTS;
					}
			...
			if (status.equalsIgnoreCase(Constants.VALID)) {
				...
				if (form.isFosCheck()) {
					fsoCheck = Constants.Y;
				} else {
					fsoCheck = Constants.N;
				}
				if (form.getAepfCheck().equals(Constants.Y)) {
					fsoCheck = Constants.Y;
				} else {
					if (form.isFosCheck()) {
						fsoCheck = Constants.Y;
				...
				preparedStatement.setInt(intCol++, form.getParentId());
				preparedStatement.setString(intCol++, Constants.ActiveState);
				...
				preparedStatement.setString(intCol++, form.getIphoneActRightString());
	...
	public String checkAvailability(String circleCode, String strCode,
		...
		String RETURN_CONSTANT = Constants.VALID;
		try {
			connection = DBConnection
					.getDBConnection(SQLConstants.DISTPORTAL_JNDI_NAME);
			String query= BusinessUserInterface.CHECK_Avilable_User;
			...
			if (resultSet.next()) {
				if (resultSet.getInt(1) > 0) {
					RETURN_CONSTANT = Constants.CODE_EXIST;
				} else {
					RETURN_CONSTANT = Constants.VALID;
				}
			}
		} catch (SQLException e) {
			RETURN_CONSTANT = Constants.INVALID;
		} finally {
			try {
				DBConnectionUtil.closeDBResources(connection,
		...
		logger
				.debug("Exiting method checkAvailability() in class RegistrationOfAllUsersDAO");
		return RETURN_CONSTANT;
	}

	/**
	 * This method checks if the registered number which we are trying to insert in the
	 * table , is already there in the table or not.
	 * 
	 * 
	 * @param circleCode :
	 ...
	 * @return Sting constant
	 */
	public String checkRegNoAvail(String regNo, int typeofuser) throws DAOException {
		logger
				.debug("Entering method checkRegNoAvail() in class RegistrationOfAllUsersDAO . Parameter Used , Registered Number : "
						+ regNo);
		PreparedStatement preparedStatement = null;
		Connection connection = null;
		ResultSet resultSet = null;
	...
	public List getBussinessUserList(String strCircleId, int typeOfUser,
		...
		try {
			...
			if (typeOfUser < 0) {
				...
				// only active users
				if(identifier.equalsIgnoreCase(Constants.singleUpload) || identifier.equalsIgnoreCase(Constants.bulkUpload) )
				{
					
					if (search.equals(Constants.all)) {
						if((typeOfUser==Constants.ED)){
					...
					} else if (search.equals(Constants.Code)) {
						if((typeOfUser==Constants.ED)){
							prepareStatement = connection
					...
					} else if (search.equals(Constants.Name)) {
						if((typeOfUser==Constants.ED)){
								prepareStatement = connection
					...
					} else if (search.equals(Constants.regNumber)) {
			...
			new DAOException("Exception occur while getting all users.", e);
		} catch (SQLException e) {
...
//			if(admin == Constants.CIRCLE_ADMIN){
//				query += " AND BASE_LOC NOT IN (?)";
//			}
//			if(admin == Constants.Zone_User){
//				query += " AND BASE_LOC NOT IN (?,?)";
//			}
			
			query += BusinessUserInterface.SEARCH_USER_TYPE_ALL_ORDER_BY;
...
//			}
			...
			while (resultSet.next()) {
				dto = new RegistrationOfAllDTO();
				dto.setLevelId(resultSet.getInt("LEVEL_ID"));
				dto.setLevelName(resultSet.getString("LEVEL_NAME"));
				//dto.setStatus(resultSet.getString("ACC_LEVEL"));
...
//				}
		...
		}catch (com.ibm.virtualization.recharge.exception.DAOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			new DAOException("Exception occur while getting all users type.", e);
		}
		catch (SQLException e) {
			e.printStackTrace();
			new DAOException("Exception occur while getting all users type.", e);
	 ...
	 * @return String
	 */
	public String getBaseLoc(int masterid) throws DAOException{
		logger
		...
		try {
			...
			//prepareStatement.setString(1, Constants.ActiveState);
			prepareStatement.setInt(1, masterid);
			resultSet = prepareStatement.executeQuery();
			while (resultSet.next()) {
				baseLoc = resultSet.getString("BASE_LOC");
			}
		}  catch (SQLException e) {
			new DAOException("Exception occur while getting base loc.", e);
		} catch(Exception ex){
			ex.printStackTrace();
		}
		finally {
	...
	public RegistrationOfAllDTO getUserProfile(int intId) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				Dto.setIphoneActRightString(resultSet.getString("IPHONE_ACT_RIGHTS"));
			...
			while (resultSet1.next()) {
				if (resultSet1.getInt("LOC_MSTR_ID") == Constants.City) {
					if(resultSet1.getInt("STATUS")==Constants.Active)
					{
				...
				if (resultSet1.getInt("LOC_MSTR_ID") == Constants.Zone) {
					if(resultSet1.getInt("STATUS")==Constants.Active)
					{
			...
			if (Dto.getBaseLoc().equals(Constants.strCircle)
					|| Dto.getBaseLoc().equals(Constants.strZone)
					|| Dto.getBaseLoc().equals(Constants.strCity)) {
...
//				 Setting empty list
			...
			if (Dto.getBaseLoc().equals(Constants.strZone)
					|| Dto.getBaseLoc().equals(Constants.strCity)) {

				Dto.setZoneList(locDao.getLocationList(Dto.getCircleCode(), Constants.Zone,Constants.PAGE_FALSE,0,0));
//				 Setting empty list
			...
			if (Dto.getBaseLoc().equals(Constants.strCity)) {
	...
	public void deleteMapping(int intId, int typeOfUser,RegistrationOfAllDTO regDTO) throws DAOException {
		...
		try {
				...
				if(rs.next())
				{
					...
					if(rs1.getInt("LOC_ID")!=0)
					{
						...
						if(rs1.getString("BASE_LOC").equalsIgnoreCase(rs.getString("BASE_LOC")))
						{
							...
							if(rs1.getInt("MASTER_ID")==Constants.TM){
								locationlist = locDao.getLocationList(String.valueOf(rs.getInt("LOC_ID")), Constants.City, 0, 0, 0);
							...
							if(rs1.getInt("MASTER_ID")==Constants.ZBM){
						...
						if(rs1.getInt("MASTER_ID")==Constants.SALES_HEAD){
						...
						if(rs1.getInt("MASTER_ID")==Constants.CEO){
			...
			throw new DAOException(
					"Exception occured while deleting the maping .", e);
		} catch (Exception e) {
			throw new DAOException(
					"Exception occured while deleting the maping .", e);
		} finally {
...
//				 updating registered number
				String regStatus = Constants.VALID;
				if(form.getStatus().equals(Constants.ActiveState))
				{
					pstmt2 = con
					...
					if (resultSet.next()) {
						if (resultSet.getInt(1) == 2 ) {
							regStatus = Constants.CODE_EXIST;
						} else {
							regStatus = Constants.VALID;
						}
					}
				}
				
				if(regStatus.equalsIgnoreCase(Constants.VALID)||form.getStatus().equals(Constants.InActive))
				{
				...
				pstmt.setString(intCol++, form.getIphoneActRightString());
				pstmt.setInt(intCol++, form.getParentId());
				...
				pstmt.close();
				con.close();
				RETURN_CONSTANT = Constants.UPDATED;
				logger
...
//				 only active users
	...
	public String aepfcheck(String circleCode) throws DAOException {
		...
		String aepfcheck = "";
		try {
			con = DBConnection.getDBConnection(SQLConstants.PRODCAT_JNDI_NAME);
			pstmt = con.prepareStatement(CircleMasterInterface.AEPF_CHECK);
			...
			throw new DAOException("Error occurs in method aepfcheck.", e);
		} catch (DAOException e) {
			throw new DAOException("Error occurs in method aepfcheck.", e);
		} finally {
			try {
...
/** updating the location details of business users **/
	...
	public int updateLocIdOfBizUserList (List bizUserList , int locId)throws DAOException
	{
		int update = 0;
		Connection conn=null;
		PreparedStatement psmt=null;
		...
		try{
			...
			psmt = conn.prepareStatement(BusinessUserInterface.UPDATE_BIZ_LIST_LOC_ID.replaceAll("<DATA_LIST>", strterr));
			psmt.setInt(1,locId);
			update=psmt.executeUpdate();
			conn.commit();
	...
	public ArrayList getBizUserList (String[] bizUserList , String circleCode)throws DAOException{
		Connection conn=null;
		PreparedStatement psmt=null;
		ResultSet rs= null;
		ArrayList bizUser= new ArrayList();
		try{
			String bizUsers =Utility.arrayToString(bizUserList,null);
			conn = DBConnection
			.getDBConnection(SQLConstants.DISTPORTAL_JNDI_NAME);
			psmt = conn.prepareStatement(BusinessUserInterface.SELECT_BIZ_LIST.replaceAll("<DATA_LIST>", bizUsers));
			psmt.setString(1,circleCode);
			rs= psmt.executeQuery();
		...
		while(rs.next()){
			...
			regDTO.setContactNo(rs.getString("MOBILE_NO"));
			
			if(rs.getString("USER_NAME")!= null){
				String label= rs.getString("USER_NAME").concat("-").concat(rs.getString("MOBILE_NO"));
				regDTO.setBusinessUserName(label);
			bizUser.add(regDTO);
		}
		}
	}catch (SQLException e) {
...
/* updating the location details of business users **/
	...
	public int updateLocIdOfBizUserLists(String[] bizUserList , int locId, String circleCode,int parentId,String updatedBy)throws DAOException
	{
		int update = 0;
		Connection conn=null;
		ResultSet rs= null;
		...
		boolean connCommitStatus = false;
		ArrayList bizUsers= new ArrayList();
		int userId;
		try{
			
			String updatedbizUserList =Utility.arrayToString(bizUserList,null);
			conn = DBConnection
			.getDBConnection(SQLConstants.DISTPORTAL_JNDI_NAME);
			connCommitStatus=conn.getAutoCommit();
			conn.setAutoCommit(false);
			
			psmt1 = conn.prepareStatement(BusinessUserInterface.GET_DOWN_HIERARCHY_BIZ_USERS.replaceAll("<DATA_LIST>", updatedbizUserList));
			rs=	psmt1.executeQuery();			
				while(rs.next()){
					userId=rs.getInt("DATA_ID");
					bizUsers.add(String.valueOf(userId));
					
					//regDTO.setBussinessUserId(String.valueOf(rs.getInt("DATA_ID")));			
//					regDTO.setLocId(rs.getInt("LOC_ID"));					
//					regDTO.setParentId(rs.getInt("PARENT_ID"));
					//bizUsers.add(regDTO);
				}
				psmt = conn.prepareStatement(BusinessUserInterface.UPDATE_BIZ_LIST_PARENT_IDS.replaceAll("<DATA_LIST>", updatedbizUserList));
				psmt.setInt(1, parentId);
			int result=	psmt.executeUpdate();
				conn.commit();
				
				String bizUsersArray [] = (String []) bizUsers.toArray (new String [0]);
				String strterr =Utility.arrayToString(bizUsersArray,null);
				if(result>0){												
						psmt2 = conn.prepareStatement(BusinessUserInterface.UPDATE_BIZ_LIST_LOC_ID.replaceAll("<DATA_LIST>", strterr));
						psmt2.setInt(1,locId);
						psmt2.setString(2,circleCode );
						psmt2.setString(3,updatedBy );
						update=psmt2.executeUpdate();
						conn.commit();
				}
		}catch (SQLException e) {
	...
	public ArrayList getmoveBizUserList(int masterId , String locId)throws DAOException
	{
		...
		try{
			...
			psmt.setString(3,Constants.ActiveState);
			rs=psmt.executeQuery();
			while(rs.next())
			{
				RegistrationOfAllDTO regDTO = new RegistrationOfAllDTO();
				...
				regDTO.setLocId(rs.getInt("LOC_ID"));
				regDTO.setRegNumber(rs.getString("MOBILE_NO"));
				regDTO.setRegNumberLabel(regDTO.getBusinessUserName()+"-"+regDTO.getRegNumber());
...
/** *  
 * getting list of childern business users for given parent and of same location id
 * created by Nitesh
 * **/
	
	public ArrayList getBizUserChildernList(String parentId , String locId)throws DAOException
	{
		logger.debug("Entering getBizChildUserList() method of RegistrationOfAllUsersDAO.");
		ArrayList moveUsers=new ArrayList();
		Connection conn=null;
		PreparedStatement psmt=null;
		ResultSet rs = null;
		...
		try{
			...
			psmt = conn.prepareStatement(BusinessUserInterface.GET_CHILDERN_LIST);
			psmt.setInt(1,Integer.parseInt(locId) );			
			psmt.setInt(2,Integer.parseInt(parentId));
			psmt.setString(3,Constants.ActiveState);
			rs=psmt.executeQuery();
			while(rs.next())
			{
			...
			throw new DAOException(
					"Excetion Occured During call of getBizChildUserList() method ",
					e);
		} finally {
			try {
				DBConnectionUtil.closeDBResources(conn, psmt,rs);
		...
		logger.debug("Exiting getBizChildUserList() method of RegistrationOfAllUsersDAO.");
		return moveUsers;
	}
...
/** getting list of users which belong to a given circle **/
	...
	public String getMSISDN(int userId)throws DAOException{
		Connection conn=null;
		PreparedStatement psmt=null;
		ResultSet rs = null;
		String msisdn= null;
		try{
			conn = DBConnection
			.getDBConnection(SQLConstants.DISTPORTAL_JNDI_NAME);
			psmt = conn.prepareStatement(BusinessUserInterface.GET_MSISDN);
			psmt.setInt(1,userId );
			rs=psmt.executeQuery();
			while(rs.next()){
				msisdn= rs.getString("MOBILE_NO");
			}
		}catch (SQLException e) {
			throw new DAOException(
					"Excetion Occured During call of getBizUserListInLocList() method ",
	...
	public ArrayList getBizUserList(RegistrationOfAllDTO regDTO)throws DAOException
	{
		ArrayList UserIdList=new ArrayList();
		Connection conn=null;
		PreparedStatement psmt=null;
		...
		try{
			...
			String query= BusinessUserInterface.GET_SUB_BIZ_LIST_FOR_MOVE;
			if(regDTO.getParentId()!= 0){
				query+= " AND PARENT_ACCOUNT = ?";
				psmt = conn.prepareStatement(query);
				psmt.setString(1, regDTO.getCircleCode());
				psmt.setString(2,Constants.ActiveState);
				psmt.setInt(3, regDTO.getParentId());
				
			}
			else if(regDTO.getLocId()!=0 ){
				query+= "AND ACCOUNT_LEVEL = ? AND CITY_ID = ? ";
				psmt = conn.prepareStatement(query);
				psmt.setString(1, regDTO.getCircleCode());
				psmt.setString(2,Constants.ActiveState);
				psmt.setInt(3, regDTO.getTypeOfUserId());
				psmt.setInt(4, regDTO.getLocId());
			}
			
			else{
				query+="AND ACCOUNT_LEVEL = ? ";
				psmt = conn.prepareStatement(query);
				psmt.setString(1, regDTO.getCircleCode());
				psmt.setString(2,Constants.ActiveState);
				psmt.setInt(3, regDTO.getTypeOfUserId());
			...
			while(rs.next())
			{
				...
				dto.setCode(rs.getString("USER_CODE"));
				dto.setContactNo(rs.getString("MOBILE_NO"));
				if(rs.getString("USER_NAME")!= null){
				String label= rs.getString("USER_NAME").concat("-").concat(rs.getString("MOBILE_NO"));
				dto.setBusinessUserName(label);
				}
				UserIdList.add(dto);
...
/** updating the circle of business users **/
	...
	public ArrayList parentList(int masterId , String cityId, String zoneId, String circleCode, String HubCode)throws DAOException
	{
		...
		try{
			...
			if(masterId==Constants.ED)
			{
				psmt = conn.prepareStatement(BusinessUserInterface.GET_PARENT_LIST_FOR_ED);
				psmt.setString(1,HubCode);
				
			}else if(masterId==Constants.CEO || masterId==Constants.SALES_HEAD){
				
				psmt = conn.prepareStatement(BusinessUserInterface.GET_PARENT_LIST_FOR_CEO_SALESHEAD);
				psmt.setString(1,circleCode);
				
			}else if(masterId==Constants.ZBM || masterId==Constants.ZSM){
				
				psmt = conn.prepareStatement(BusinessUserInterface.GET_PARENT_LIST);
				psmt.setInt(1,Integer.parseInt(zoneId) );
				
			}else{
				
				psmt = conn.prepareStatement(BusinessUserInterface.GET_PARENT_LIST);
				psmt.setInt(1,Integer.parseInt(cityId) );
