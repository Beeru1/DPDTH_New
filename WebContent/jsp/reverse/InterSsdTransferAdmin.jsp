...
function getProductList()
    ...
    function validateDist()
    {
     ...
     var toDist = document.getElementById("toDistId").value;
	    if(fromDist == toDist )
		{
		 	alert("From and To Distributor can not be same.");
		 	document.getElementById("toDistId").value = "-1";
	    }		
    }
	...
	function OpenSerialWindow()
	{
	  ...
	  var url="viewStbList.do?methodName=viewSerialNoList&fromDist="+fromDist+"&productId="+productId+"&circleId="+circleId;
   ...
   function OpenSerialWindow2(productId,newSelected)
	...
	function getFromDistName()
	{
	    ...
	    var busCategory = document.getElementById("businessCat").value;
	 	...
	 	if(levelId!=''){
			var url = "interSsdTranfrAdmin.do?methodName=getFromDistList&parentId="+levelId+"&circleId="+circleId+"&busCategory="+busCategory;
			if(window.XmlHttpRequest) 
			{
				req = new XmlHttpRequest();
			}else if(window.ActiveXObject) 
	...
	function availableQty(){
	    ...
	    if(productId!="-1" && productId!=""){
			var url = "interSsdTranfrAdmin.do?methodName=getAvailableQty&fromDist="+fromDist+"&productId="+productId+"&circleId="+circleId;
			if(window.XmlHttpRequest) 
			{
				req = new XmlHttpRequest();
			}else if(window.ActiveXObject) 
...
function insertData(){
	...
	else if(document.getElementById("toTsmId").value == ""){
		alert("Please select To TSM!");
		return false;
	}else if(document.getElementById("toDistId").value == "-1"){
		alert("Please select To Distributor!");
		return false;
	}
	...
	while (i+1<rowCount)
		{
			var x = document.getElementById("dataTable").rows[i+1].cells;
				...
				jsArrSerialNo[i] = x[1].getElementsByTagName("input")[0].value;
	...
	document.forms[0].jsArrprodId.value=jsArrProductId;
  	...
  	if(confirm("Are you sure to transfer the stock?")){
		document.forms[0].action="interSsdTranfrAdmin.do?methodName=insertData&fromDist="+fromDist+"&circleId="+circleId+"&toDist="+toDist;
	...
	<TABLE width="80%" border="0" cellpadding="1" cellspacing="1"
		...
		<td colspan='2'>&nbsp;
			...
			<tr>
				...
				<html:select property="businessCat" style="width:150px" onchange="getProductList();getFromTsmName();getTsmName();">
...
<TABLE width="60%" border="0" cellpadding="1" cellspacing="1"
	...
	<tr>
				<td align="center">
				<input type="button" name="interssdButton" value="Transfer Stock" onclick="insertData();">
				</td>
