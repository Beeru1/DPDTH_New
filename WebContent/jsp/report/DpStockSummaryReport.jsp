...
<SCRIPT language="javascript" type="text/javascript"><!-- 
	...
	function exportToExcel()
	{
		...
		var url = "stockSummaryReport.do?methodName=getReverseLogisticReportExcel&fromDate="+fromDate+"&toDate="+toDate+"&circleId="+circle+"&accountId="+accountId+"&distAccId="+distAccId+"&selectedProductId="+productid;
	...
	function getTsmName() 
	{
	    ...
	    var levelId = document.getElementById("circleID").value;
		 if(levelId=='0'){
		 	document.getElementById("selectedProductId").disabled = true;
		 	document.getElementById("selectedProductId").value="0";
		 }else{
			document.getElementById("selectedProductId").disabled = false;
			document.getElementById("selectedProductId").value="0";
		 }
	    if(levelId!=''){
			var url = "stockSummaryReport.do?methodName=initTsmReport&levelId="+levelId;
	...
	function getDistName()
	{
	    ...
	    var circleId = document.getElementById("circleid").value;
	 	...
	 	if(levelId!=''){
			var url = "stockSummaryReport.do?methodName=initDistReport&parentId="+levelId+"&circleId="+circleId;
			if(window.XmlHttpRequest) 
			{
				req = new XmlHttpRequest();
			}else if(window.ActiveXObject) 
...
<TABLE cellspacing="0" cellpadding="0" border="0" width="100%" 	height="100%" valign="top">
	...
	<html:form name="StockSummaryReportBean" action="/reverseLogisticReport" type="com.ibm.dp.beans.StockSummaryReportBean" >
		...
		<td>
			<TABLE width="570" border="0" cellpadding="5" cellspacing="0" class="text">
				<TR>
					...
					<IMG src="${pageContext.request.contextPath}/images/StockSummarReportNew.jpg"
				...
				<td width="60%">	
					<logic:match value="A" property="showCircle" name="StockSummaryReportBean">		
						<html:select styleId="circleid" property="circleid"
							name="StockSummaryReportBean" style="width:150px"
							style="height:20px" onchange="javascript:getTsmName();">
							<html:option value="">
								<bean:message bundle="view" key="product.selectcircle" />
							</html:option>
							<logic:notEmpty name="StockSummaryReportBean" property="arrCIList">
								<html:optionsCollection name="StockSummaryReportBean"	property="arrCIList" />
							</logic:notEmpty>
						</html:select>
						</logic:match>
						
						<logic:match value="N" property="showCircle" name="StockSummaryReportBean">		
						<html:select styleId="circleid" property="circleid" name="StockSummaryReportBean" style="width:150px"
							style="height:20px" disabled="true"  onchange="javascript:getTsmName();">
							<html:option value="">
								<bean:message bundle="view" key="product.selectcircle" />
							</html:option>
							<logic:notEmpty name="StockSummaryReportBean"
								property="arrCIList">
								<html:optionsCollection name="StockSummaryReportBean"
									property="arrCIList" />
							</logic:notEmpty>
						</html:select>
						</logic:match>
				...
				<TR>
					...
					<TD >				   
						<html:select property="accountId" onchange="javascript:getDistName()" name="StockSummaryReportBean">
							<html:option value="">- Select a TSM -</html:option>
							<logic:notEmpty property="tsmList" name="StockSummaryReportBean">
							<html:option value="0">Select all TSMs</html:option>
						
						<html:optionsCollection name="StockSummaryReportBean"	property="tsmList" label="accountName" value="accountId"/>
						</logic:notEmpty>
						</html:select>
					</TD>
				</TR>
				...
				<TR>
					...
					<TD >				   
						<html:select property="distAccId" name="StockSummaryReportBean">
							<html:option value="">- Select a Distributor -</html:option>
							<logic:notEmpty property="distList" name="StockSummaryReportBean">
							<option value="0">Select all Distributors</option>
						<html:optionsCollection name="StockSummaryReportBean"	property="distList" label="distAccName" value="distAccId"/>
						</logic:notEmpty>
						</html:select>
					</TD>
				</TR>
				...
				<logic:notEmpty property="productList" name="StockSummaryReportBean"><html:options labelProperty="productName" property="productId"
				...
				<TR>
					<TD align="center"><b class="text pLeft15"><bean:message bundle="view"
						...
						size="15" maxlength="10" name="StockSummaryReportBean" />
						<script language="JavaScript">
						new tcal ({
							// form name
							'formname': 'StockSummaryReportBean',
							// input name
	...
	<logic:equal value="N" name="StockSummaryReportBean" property="showTSM">
		<script>
		 		document.getElementById("accountId").disabled = "true";
		 		
		 </script>	
	</logic:equal>
	<logic:equal value="N" name="StockSummaryReportBean" property="showDist">
		<script>
