...
<SCRIPT language="javascript" type="text/javascript">
	...
	function findForTransId(transId){
		document.forms[0].action="TransactionRptAction.do?methodName=searchRechargeTransactionReportData&trasactionId="+transId ;
		document.forms[0].method="post";
		document.forms[0].submit();
	}
	
	function submitData(nextPage,searchType,searchValue,transactionType,status,startDt,endDt){
		document.forms[0].action="TransactionRptAction.do?methodName=searchRechargeTransactionReportData&page="
				+ nextPage + "&searchType=" + searchType + "&searchValue="
				+ searchValue+ "&transactionType=" + transactionType
				+ "&status=" + status + "&startDt=" 
				+ startDt + "&endDt=" + endDt;
document.getElementById("page").value="";	
	...
	function validate(){
		...
		if(form.circleId.value=='-1'){
			alert('<bean:message bundle="error" key="error.report.circleid" />');
			form.circleId.focus();
			return false;
		}
		...
		if(document.getElementById("searchFieldName").value=="0" ){
	   	...
	   	document.getElementById("page").value="";		
		document.getElementById("methodName").value="searchRechargeTransactionReportData";
		return true;
	}  // Validate Form Ends
	...
	function exportDataToExcel(){
		document.getElementById("methodName").value="exportCustomerRechargeReport";
		document.forms[0].submit();
	}
	function resetAllData(){
		resetAll();
		document.getElementById("methodName").value="initCustomerRechargeReport";	
		document.forms[0].submit();
	}
	...
	<TR valign="top">
		...
		<TD valign="top" width="100%" height="100%"><html:form
			...
			<TABLE width="680" border="0" cellpadding="5" cellspacing="0"
				...
				<TR>
					<TD width="290" class="text pLeft15" nowrap><STRONG><FONT
						color="#000000"><bean:message bundle="view"
						key="report.circle_name" /></FONT><FONT color="RED">*</FONT> :</STRONG> <FONT
						color="#003399"> <logic:equal
						name="TransactionRptA2AFormBean" property="flagForCircleDisplay"
						value="true">
						<html:select property="circleId" disabled="true" styleClass="w130"
							tabindex="1">
							<html:option value="-1">
								<bean:message bundle="view" key="application.option.select" />
							</html:option>
							<html:optionsCollection name="TransactionRptA2AFormBean"
								property="circleList" label="circleName" value="circleId" />
						</html:select>
					</logic:equal> <logic:notEqual name="TransactionRptA2AFormBean"
						property="flagForCircleDisplay" value="true">
						<html:select property="circleId" styleClass="w130" tabindex="1">
							<html:option value="-1">
								<bean:message bundle="view" key="application.option.select" />
							</html:option>
							<html:optionsCollection name="TransactionRptA2AFormBean"
					...
					<TD class="text pLeft15" align="left" nowrap><FONT
						color="#000000"><STRONG><bean:message
...
'controlname': 'startDt'
					...
					<TD class="text pLeft15" align="left" nowrap colspan="3"><FONT
						color="#000000"><STRONG><bean:message
...
'controlname': 'endDt'
				...
				<tr>
					...
					<html:select property="transactionType"
						name="TransactionRptA2AFormBean" styleClass="w130" tabindex="4">
					...
					<html:select property="status" name="TransactionRptA2AFormBean"
						style="width:85" styleClass="w130" tabindex="5">
						<html:option value="0">
						...
						<html:option value="4">
							<bean:message key="report.transaction.all" bundle="view" />
						</html:option>
					</html:select></TD>
			...
			<TABLE width="680" border="0" cellpadding="5" cellspacing="0"
				...
				<TR>
					...
					<html:select property="searchFieldName" styleId="searchFieldName"
						onchange="setSearchControlDisabled()" tabindex="6"
						styleClass="w130">
						<html:option value="0">
							<bean:message bundle="view" key="application.option.select" />
						</html:option>
						...
						styleId="searchFieldValue" size="15" maxlength="20" tabindex="7" />
					&nbsp;&nbsp;&nbsp;&nbsp; <INPUT class="submit"
						...
						<logic:notEmpty name="TransactionRptA2AFormBean"
							...
							<TR align="center">
								<TD align="left" colspan="13"><INPUT class="submit"
									...
									name="export" value="Export to Excel" tabindex="10"></TD>
							</TR>
							...
							<TR align="center" bgcolor="#104e8b">
								...
								<logic:match name="TransactionRptA2AFormBean" property="transactionType"
											value="RECHARGE">
								<TD align="center" bgcolor="#cd0504"><SPAN
									class="white10heading mLeft5 mTop5"><bean:message
								...
								<logic:notMatch name="TransactionRptA2AFormBean" property="transactionType" 
								value="RECHARGE">
								<TD align="center" bgcolor="#cd0504"><SPAN
									class="white10heading mLeft5 mTop5"><bean:message
							...
							<logic:iterate id="transaction" name="TransactionRptA2AFormBean"
								...
								<c:if  test='${transactionType eq "POSTPAID_DTH"}'>
									...
									<TD class="text" align="center" nowrap><bean:write
										name="transaction" property="reason" /> </TD><logic:match 
										name="transaction" property="transactionType" value="RECHARGE">	
									...
									<TD class="text" align="center"><logic:notMatch name="transaction" property="selfcareStatus" value="null">
									...
									<TD class=" height19" align="right"><input Type="button"
										onClick="findForTransId(${transaction.transactionId});"
										value="<bean:message bundle="view" key="application.view" />"
										class="submit" /></TD>
							...
							<TR align="center">
								<TD align="center" bgcolor="#daeefc" colspan="13"><FONT
								 ...
								 <c:if test="${PRE>=1}">
											<a href="#"
												onclick="submitData('<c:out value='${PRE}' />','<c:out value='${searchType}' />','<c:out value='${searchValue}' />','<c:out value='${transactionType}' />','<c:out value='${status}' />','<c:out value='${startDt}' />','<c:out value='${endDt}' />');"><
											Prev</a>
								 </c:if>
								 ...
								 <c:if test="${PAGES>LinksPerPage}">
									...
									<c:forEach var="item" step="1" begin="${start_page}"
										...
										<c:choose>
											...
											<c:otherwise>
												<a href="#"
														onclick="submitData('<c:out value="${item}"/>','<c:out value='${searchType}' />','<c:out value='${searchValue}' />','<c:out value='${transactionType}' />','<c:out value='${status}' />','<c:out value='${startDt}' />','<c:out value='${endDt}' />');"><c:out
														value="${item}" /></a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								...
								<c:if test="${NEXT<=PAGES}">
											<a href="#"
												onclick="submitData('<c:out value='${NEXT}' />','<c:out value='${searchType}' />','<c:out value='${searchValue}' />','<c:out value='${transactionType}' />','<c:out value='${status}' />','<c:out value='${startDt}' />','<c:out value='${endDt}' />');">Next></a>									
								</c:if>
