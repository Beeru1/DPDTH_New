...
function getChange(Id)
{
	//var Id = document.getElementById("businessCategory").value;   
	 var url="initCreateAccountItHelp.do?methodName=getParentCategory&OBJECT_ID="+Id+"&condition=category";
...
<SCRIPT language="javascript" type="text/javascript"> 
		function submitData(nextPage,searchType,searchValue,circleId,circleIdList,accountLevel){
		document.forms[0].action="initCreateAccountItHelp.do?methodName=getParentAccountList&page="
				+ nextPage + "&searchType=" + searchType + "&searchValue="
				+ searchValue + "&circleId=" + circleId + "&circleIdList=" + circleIdList + "&accountLevel=" 
				+ accountLevel;
	...
	function getAccount(){
		...
		//if((window.opener.document.forms[0].circleId.value=="0" || window.opener.document.forms[0].circleId.value=="") && (window.opener.document.forms[0].circleIdList.value=="0" || window.opener.document.forms[0].circleIdList.value=="")){
		if((window.opener.document.forms[0].circleId.value=="") && (window.opener.document.forms[0].circleIdList.value=="0" || window.opener.document.forms[0].circleIdList.value=="")){
	...
	if(window.opener.document.forms[0].accountLevel.value == "3" || window.opener.document.forms[0].accountLevel.value == "4" || window.opener.document.forms[0].accountLevel.value == "5" )	{
		var circleidList=window.opener.document.forms[0].circleIdList.value;
	   	if(circleidList != ""){
			for (var i=1; i < window.opener.document.forms[0].circleIdList.length; i++)
	  	 {
	   		 if (window.opener.document.forms[0].circleIdList[i].selected)
	     	 {
	     		if(selectedCircleValues !=""){
					selectedCircleValues += ",";
	     		}
	     	var selectedValCircle = window.opener.document.forms[0].circleIdList[i].value.split(",");
	    ...
	    document.getElementById("circleIdList").value=selectedCircleValues ; //window.opener.document.forms[0].circleIdList.value;
	    ...
	    var send = "methodName="+name+"&circleId="+circleid+"&circleIdList="+selectedCircleValues+"&parent="+parent;
	   return true;
	}
	function getCode(accountCode,tradeName,tradeCategoryName,parentAccount,outletType,loginName,billableName){
		...
		if(window.opener.document.forms[0].accountLevel.value == "6" && window.opener.document.forms[0].distTypeId.value == "2"){		
		window.opener.document.forms[0].depositeTypeTSM.value=accountCode;
		window.opener.document.forms[0].depositeTypeTSMAccountName.value=accountCode;
		window.opener.document.forms[0].depositeTypeTSMLoginName.value=loginName;
			
		}
		else if(window.opener.document.forms[0].accountLevel.value == "6" && window.opener.document.forms[0].distTypeId.value == "1"){		
		window.opener.document.forms[0].salesTypeTSM.value=accountCode;
		window.opener.document.forms[0].salesTypeTSMAccountName.value=accountCode;
		window.opener.document.forms[0].salesTypeTSMLoginName.value=loginName;
...
<BODY background="<%=request.getContextPath()%>/images/bg_main.gif" bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0"
	marginheight="0" onkeypress="return checkKeyPressed();" onload="setSearchControlDisabled()">
	<html:form action="initCreateAccountItHelp" focus="searchFieldName">
		...
		<html:hidden property="circleIdList" styleId="circleIdList" />
				...
				<TABLE width="100%" border="0" cellpadding="5" cellspacing="0" class="text">
					...
					<TR>
						...
						<TD>
							...
							<INPUT class="submit" onclick="return validate();" type="submit" tabindex="3" name="Submit" value="Search">
						</TD>
					</TR>
				</TABLE>
			</TD>
		...
		<TR>
			<TD width='100%'>
				<div style="DISPLAY: block">
					<DIV style="height: 350px; overflow: auto; position: relative;" width="100%">
							...
							<TR>
								<TD width='100%'>
									<TABLE width="100%" align="left" class="mLeft5">
										...
										<thead>
											<TR align="center" bgcolor="#CD0504"  style="position: relative; top:expression(this.offsetParent.scrollTop-2); color:#ffffff !important; ">
												...
												<TD align="center" bgcolor="#CD0504"><SPAN
													class="white10heading mLeft5 mTop5"><bean:message
													bundle="view" key="account.email" /></SPAN>
												</TD>

											</TR>
										</thead>
										<tbody>
										<logic:iterate id="account" name="DPCreateAccountITHelpFormBean" property="displayDetails" indexId="i">
