	...
	//var imgDir = "../cal/";	     // directory for images ... e.g. var var imgDir = "/img/"
	var imgDir = "cal/";	
	...
	{ 
		...
		this.bw=(this.ie5 || this.ie5 || this.ie4 || this.ns4 || this.ns5) 
	    return this 
	} 
    ...
    */
	...
	if (dom)
	{
		...
		document.write ("<div onclick='bShow=true'  id='calendar' onmouseover=\"document.all.close.src='"+imgDir+"close.gif'\" onmouseup=\"document.all.close.src='"+imgDir+"close.gif'\"	style='z-index:+999;position:absolute;visibility:hidden;'><table cellpadding=0 cellspacing=0 width="+((showWeekNumber==1)?250:200)+" style='font-family:verdana;font-size:10px;border-width:1px;border-style:solid;border-color: #666666;font-family:verdana; font-size:10px; font-color:#6A6DB2}' bgcolor='#6A6DB2'><tr bgcolor='#6A6DB2'><td><table cellpadding=2 cellspacing=0 width='"+((showWeekNumber==1)?248:218)+"' ><tr><td nowrap style='padding:2px;font-family:verdana; font-size:10px;' align=right><font color='#ffffff'><B><span id='caption'></span></B></font></td><td nowrap align=right><a href='javascript:hideCalendar()' onmousedown=\"document.all.close.src='"+imgDir+"close2.gif'\" onmouseover='popDownYear(); popDownMonth();'><IMG id=close SRC='"+imgDir+"close.gif' WIDTH='16' HEIGHT='14' BORDER='0' ALT='Close the Calendar' align='absmiddle'></a>&nbsp;</td></tr></table></td></tr><tr><td style='padding:5px' bgcolor=#E6E7F9><span id='content'></span></td></tr>")
			
		if (showToday==1)
		{
			document.write ("<tr bgcolor=#6A6DB2><td style='padding:5px' align=center><span id='lblToday'></span></td></tr>")
		}
	...
	var styleAnchor="text-decoration:none;color:#6A6DB2;"
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			for	(i=0; i<12;	i++) {
				...
				sHTML += "<tr><td id='m" + i + "' onmouseover='this.style.backgroundColor=\"#E6E7F9\"; this.style.borderColor=\"#666666\"' onmouseout='this.style.backgroundColor=\"\"; this.style.borderColor=\"#FFFFFF\"' style='cursor:pointer; border: 1px solid #ffffff;' bgcolor=#ffffff onclick='monthConstructed=false;monthSelected=" + i + ";constructCalendar();popDownMonth();event.cancelBubble=true'><font color=#6A6DB2>&nbsp;" + sName + "&nbsp;</font></td></tr>"
			}

			document.getElementById("selectMonth").innerHTML = "<table width=70 style='font-family:verdana; font-size:10px; border-width:1; border-style:solid; border-color:#666666; border-Top: 0px;' bgcolor='#FFFFFF' cellspacing=1 cellpadding=0 onmouseover='clearTimeout(timeoutID1)'	onmouseout='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"popDownMonth()\",100);event.cancelBubble=true'>" +	sHTML +	"</table>"
	...
	function incYear() {
		for	(i=0; i<7; i++){
			...
			{ txtYear =	"<font color=#6A6DB2>&nbsp;<B>"	+ newYear +	"</B>&nbsp;</font>" }
			else
			{ txtYear =	"<font color=#6A6DB2>&nbsp;" + newYear + "&nbsp;</font>" }
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			for	(i=(yearSelected-3); i<=(yearSelected+3); i++) {
				...
				sHTML += "<tr><td id='y" + j + "' onmouseover='this.style.backgroundColor=\"#E6E7F9\"; this.style.borderColor=\"#666666\"' onmouseout='this.style.backgroundColor=\"\"; this.style.borderColor=\"#FFFFFF\"' style='cursor:pointer; border: 1px solid #ffffff;' bgcolor=#ffffff onclick='selectYear("+j+");event.cancelBubble=true'><font color=#6A6DB2>&nbsp;" + sName + "&nbsp;</font></td></tr>"
				j ++;
			}
			...
			document.getElementById("selectYear").innerHTML	= "<table width=44 style='font-family:verdana; font-size:10px; border-width:1; border-style:solid; border-color:#666666; border-Top: 0px;' bgcolor='#FFFFFF' onmouseover='clearTimeout(timeoutID2)' onmouseout='clearTimeout(timeoutID2);timeoutID2=setTimeout(\"popDownYear();\",100)' cellspacing=1 cellpadding=0>"	+ sHTML	+ "</table>"
   ...
   function WeekNbr(n) {
	...
	function constructCalendar () {
		...
		if (showWeekNumber==1)
		{
			sHTML += "<td width=27><b>" + weekString + "</b></td><td width=1 rowspan=7 bgcolor='#6A6DB2' style='padding:0px'><img src='"+imgDir+"divider.gif' width=1></td>"
		}

		for	(i=0; i<7; i++)	{
			sHTML += "<td width='27' align='right'><font color='#6A6DB2'><B>"+ dayName[i]+"</B></font></td>"
		}
		sHTML +="</tr><tr>"
	...
	function popUpCalendar(ctl, ctl2, format, frameOffSetLeft, frameOffSetTop) {
		...
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
				...
				{
					// use user's date
					aData =	new String(ctl2.value).split(formatChar)
